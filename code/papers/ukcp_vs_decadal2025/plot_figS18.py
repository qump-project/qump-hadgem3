...
namearr = ['England and Wales_djf_tas', 'England and Wales_jja_tas','England and Wales_djf_pr', 'England and Wales_jja_pr'] 
fparr   = [1, 1, 1, 1]
ylabels = ['Anomaly ($\degree$C)', 'Anomaly ($\degree$C)', 'Anomaly (%)', 'Anomaly (%)']
nmaxa   = [58, 57, 58, 57]
...
fig = plt.figure(ifig, figsize=(18/2.54, 15/2.54) )
plt.subplots_adjust(hspace=0.27,wspace=0.27,top=0.93,bottom=0.055,left=0.095,right=0.975)

for iname,longname,fp,ylabel,nmx in zip(range(len(namearr)),namearr, fparr,ylabels,nmaxa):
    datamed  = []
    ndata    = []
    modellist= []
    ...
    for imod,model in enumerate(models):               
        ...
        if data1 is None:
            ...
            mscale=scale.mean()           
            print('> ',longname,model,'mscale=',mscale,'2.5632*mscale=',2.5632*mscale)
            ...
            dif=numpy.array(dif).T    #shape=(58,100)   
            med=numpy.array(med).T    #shape=(58,100)
    ...
    for icol in range(datamed.shape[1]):
        plt.plot(tsyr,datamed[:,icol],color=coldic[modellist[icol]],alpha=0.9,lw=1.0)
    ...
    if iname == 0:
        tit='EngWal winter Tair, T1'
        ylim=[-1.75, 1.75]
        xlim=[1960., 2018.5]

    elif iname == 1:
        tit='EngWal summer Tair, T1'
        ylim=[-2.15, 2.0]
        xlim=[1960., 2018.5]

    elif iname == 2:
        tit='EngWal winter precipitation, T1'
        ylim=[-32.0, 32.0]
        xlim=[1960., 2018.5]

    elif iname == 3:
        tit='EngWal summer precipitation, T1'
        ylim=[-30., 34.]
        xlim=[1960., 2018.5]
        
    cfreqout='\nFreq. outside P90-P10 range: %5.2f'%freqout
    plt.title(tit+cfreqout, fontsize=fs+0.5,pad=3.0)
    ax.set_ylim(ylim)
    ax.set_xlim(xlim)
    ...
    plt.ylabel(ylabel)
    loc = 'upper left'            
    leg1 = plt.legend(ll, labels, loc=loc, fontsize=fs-0.5, handlelength=1.0,borderaxespad=0.05,
                      handletextpad=0.25,labelspacing=0.20,frameon=False)

    #print('>>> ',iname,longname,'datamed.shape=',datamed.shape,modellist)
    ...
    if iname in [0,1]:
        ...
        leg2 = plt.legend(ll2, labels2, loc='lower right', fontsize=fs-1.0, frameon=False, ncol=ncol, columnspacing=0.6,
