...
for iname,longname,fp,ylabel,nmx in zip(range(5),namearr, fparr,ylabels,nmaxa):
    ...
    prob=[0.1, 0.5, 0.9]
    rrr = scipy.stats.mstats.mquantiles(datarall,prob=prob,axis=1)  #default is alphap=0.4,betap=0.4

    # difall shape = (55,1500) = (ntime,100*9+200*1+400*1) 
    ...
    plt.ylabel(ylabel)
    loc = 'upper left'        
    leg1 = plt.legend(ll, labels, loc=loc, fontsize=fsleg1, handlelength=1.0,borderaxespad=0.05,
