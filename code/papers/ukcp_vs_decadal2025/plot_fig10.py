...
freq_mean_lis1 = []  ; freq_mean_lis2 = []  ; freq_mean_lis3 = [] 
freq_arr_lis1  = []  ; freq_arr_lis2  = []  ; freq_arr_lis3  = [] 
freq_ukcp_lis1 = []  ; freq_ukcp_lis2 = []  ; freq_ukcp_lis3 = [] 
...
for iname,longname in zip(range(nv),namearr):
    for fp in fparr:
        ...
        dall, dlistok, modlist, syr, nlump = loadutils.loadDPS(longname, dpsdir, fp=fp, ensembles=ensembles, fac=fac, yearmax=yearmax)
        ...
        print('UKCP',longname,'freq_ukcp=', freq_ukcp)
        print('UKCP',longname,'freq_mean=', freq_mean)
        ...
        print('>>> ',iname,'len(freq_arr)=',len(freq_arr) )
        isort=numpy.argsort(freq_arr)
        print('>>>',longname,', fp=',fp)
        for ii in range(len(modlist)):           
             print(ii, modlist[isort[ii]], 'freq=',freq_arr[isort[ii]])
        ...
        # Linear trend estimation, only for 2to9 data.
        if fp == '2to9':
            if '_pr' in longname:
                unit='%'
            elif '_psl' in longname:
                unit='hPa'
            else:
                unit='degC'                        
...
trendfile = os.path.join(plotdir, 'trends_2to9_3glb_'+cccc+'.txt')

if saveplot:
    f1 = open(trendfile, 'w')
    head = '                                       Decadal Trends (per decade)'
    f1.write(head +'\n')
    head = 'VARIABLE                          MMDPE     UKCP      OBS   Obs_'+cprob+'%_Conf.Int'
    f1.write(head +'\n')
...
fig = plt.figure(ifig, figsize=(9/2.54, 9/2.54) )
plt.subplots_adjust(hspace=0.25,wspace=0.25,top=0.89,bottom=0.09,left=0.11,right=0.98)
...
for iname,longname,xlabel in zip(range(nv),namearr, xlabelarr):
    ...
    for ibox in range(nbox):
        ...
        print('>>> ',iname,xlabel,ibox,score_arr.shape,score_arr)
        ...
        ax.plot(xp, score_arr, lw=0,marker='o',ms=3.0, color=colbox,alpha=0.6)   
...
ax.set_xticklabels(xlabelarr, fontsize=fs-0.0)    
plt.title('Fraction of Observed Events in \n10-90% Hindcast Range',pad=3.0, fontsize=fs+1.0)   

xlims=ax.get_xlim()
ylims=ax.get_ylim()
xleg=xlims[0]+0.73*(xlims[1]-xlims[0])
dxtxt=0.055*(xlims[1]-xlims[0])
dxbox=0.045*(xlims[1]-xlims[0])
