...
namearr=['northerneurope_tas_djf', 'northerneurope_tas_jja', 'northerneurope_pr_djf', 'northerneurope_pr_jja']
...
for iname,longname in enumerate(namearr):
    ...
    if longname == 'northerneurope_tas_djf':
        tit1='NEU winter Tair spread'                
        obslab='Obs.'   
        yunit='($\degree$C)'
        ymin=2.5
        ymx=5.9

        txtfile=os.path.join(ukcpdir, 'ci80_tas_djf_NEU.txt') 
    ...
    elif longname == 'northerneurope_tas_jja':
        tit1='NEU summer Tair spread'
        obslab='Obs.'
        yunit='($\degree$C)'
        ymin=1.0
        ymx =3.0     

        txtfile=os.path.join(ukcpdir, 'ci80_tas_jja_NEU.txt') 
        print('Input data from: ',txtfile)
        usecols = list(range(4,14))
        d = numpy.genfromtxt(txtfile, skip_header=2, usecols=usecols)
        obs3 = [d[0], d[1], d[2]]
    ...
    elif longname == 'northerneurope_pr_djf':
        tit1='NEU winter precipitation spread'
        obslab='Obs'
        yunit='(%)'
        ymin = 19.0
        ymx  = 37.0    

        txtfile=os.path.join(ukcpdir, 'ci80_pr_djf_NEU.txt') 
        print('Input data from: ',txtfile)
        usecols = list(range(4,14))
        d = numpy.genfromtxt(txtfile, skip_header=2, usecols=usecols)
        obs3 = [d[0], d[1], d[2]]
    ...
    elif longname == 'northerneurope_pr_jja':
        tit1='NEU summer precipitation spread'   
        obslab='Obs'
        yunit='(%)'
        ymin = 19.0
        ymx  = 51.5    

        txtfile=os.path.join(ukcpdir, 'ci80_pr_jja_NEU.txt') 
        print('Input data from: ',txtfile)
        usecols = list(range(4,14))
        d = numpy.genfromtxt(txtfile, skip_header=2, usecols=usecols)
        obs3 = [d[0], d[1], d[2]]
